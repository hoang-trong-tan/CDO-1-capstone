...
| API Ingress | Internal Application Load Balancer (ALB) | Tiếp nhận HTTP alerts từ AlertManager nội bộ, hỗ trợ routing theo path và authentication. | ~$22.27/month (Base + LCU) |
| Database (Sandbox) | RDS PostgreSQL Single-AZ (`db.t3.micro`) | Lưu cấu hình sandbox. Single-AZ nhỏ nhất để tối ưu chi phí (db.t3.micro + 20 GB gp3 storage). | ~$15.44/month |
...
Để chống rủi ro ArgoCD tự revert patch nóng (race condition), nhóm áp dụng giải pháp **Sync Suspension**: tạm tắt sync policy của ArgoCD qua API $\rightarrow$ patch cluster $\rightarrow$ commit Git cấu hình limits mới $\rightarrow$ bật lại sync & force sync.
...
|---|---|---|
| **Cost / tenant / month** | **$101.65** | **$90.53** |
...
*Giải trình:* Chi phí Hybrid ($101.65/tenant) cao hơn Serverless do EKS Control Plane và VPC Endpoints cố định, nhưng tối ưu compute dùng chung ở quy mô lớn. Latency của Hybrid thấp hơn do in-cluster API calls so với overhead cold start của Lambda/Step Functions.

### 3.3 Weakness chấp nhận

*   **Trade-off 1 (Chi phí cố định):** Chênh lệch $11.12/tenant/month (~12.3%) so với Serverless được chấp nhận để giữ mọi thành phần trong in-cluster security boundary. *Mitigation:* Nếu AlertManager nội bộ, có thể loại bỏ ALB để giảm tiếp $11.13/tenant.
...
*   **Xác thực (Validate Context):** Mọi request alert bắt buộc mang header `X-Tenant-Id`. FastAPI Middleware áp dụng nguyên tắc Zero Trust: validate `tenant_id` với target namespace và registry trong DynamoDB. Nếu phát hiện mismatch (ví dụ: tenant-payment yêu cầu restart pod ở tenant-checkout namespace), request sẽ bị chặn lập tức và trả về `403 Forbidden` kèm ghi nhận log bảo mật `SECURITY_VIOLATION`.
...
2.  **Idempotency Lock & Cooldown:** DynamoDB Conditional Write khóa sự cố dựa trên key `tenant_id#namespace#service#alert_name#action_type`. Alert trùng lặp gửi đến trong thời gian cooldown sẽ bị bỏ qua và đánh dấu `SUPPRESSED_DUPLICATE`.
...
*   **Hành động (Remediation):** Self-Heal Engine thực hiện patch `resources.limits` của Deployment (tăng RAM × 1.5, CPU × 1.5), commit manifest mới vào GitOps repo. ArgoCD tự động reconcile và rollout Pod mới.
...
|---|---|---|---|
| **Scale-Up** | CPU trung bình **> 70%** hoặc Alert SQS Queue depth **> 1000 msgs** | 5 phút (CPU) / 2 phút (SQS) | HPA: +1 Pod replica (max 10 Pods) / Engine: +2 Workers |
| **Scale-Down** | CPU trung bình **< 40%** hoặc Alert SQS Queue depth **< 100 msgs** | 10 phút liên tục (cooldown) | HPA: -1 Pod / Engine: Rollback/Scale down về baseline |
...
| **Scale-Down (consolidation)** | Node có utilization thấp, các Pod có thể reschedule sang node khác an toàn | **< 30 giây** | Drain node, terminate instance, giải phóng chi phí |
...
| **SQS queue depth** | `> 1000 messages` | `< 100 messages` | 2 phút | Self-Heal Engine: ±2 Workers |
